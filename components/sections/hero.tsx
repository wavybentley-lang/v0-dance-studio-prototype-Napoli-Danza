...
export function HeroSection({ mouseX = 0.5, mouseY = 0.5 }: { mouseX?: number; mouseY?: number }) {
  ...
  return (
    <section className="relative flex min-h-screen items-center overflow-hidden" style={{ background: "radial-gradient(ellipse at 70% 30%, #5C1535 0%, #3D0D26 50%, #250818 100%)" }}>
      ...
      <div className="absolute inset-y-0 right-0 z-0 w-full [background-position:center_8%] sm:[background-position:center_15%] lg:w-[62%]" style={{ backgroundImage: "url('/heroimage.jpg')", backgroundPosition: "center 15%", backgroundSize: "cover" }}>
      ...
      <div className="relative z-10 ml-0 max-w-7xl px-4 pt-32 pb-24 text-left sm:px-6 sm:pt-36 sm:pb-28 lg:mx-auto lg:w-1/2 lg:px-8 lg:py-0">
        <h1 className="mb-6 text-left font-serif text-5xl leading-[0.96] font-bold text-balance text-[#F5F5F5] italic sm:text-6xl md:text-7xl lg:text-8xl">
          <span className="animate-clip-reveal inline-block">Ogni grande ballerino</span>{" "}
          <span className="animate-clip-reveal inline-block" style={{ animationDelay: "0.15s" }}>nasce da un</span>{" "}
          <span className="animate-clip-reveal inline-block" style={{ animationDelay: "0.35s" }}>sogno.</span>
        </h1>
        ...
        <div className="animate-clip-reveal flex flex-col justify-start gap-4 sm:flex-row" style={{ animationDelay: "0.7s" }}>
          ...
          <a href="#contatti" className="animate-pulse-glow rounded-full border border-[#F5F5F5]/60 bg-card/70 px-8 py-4 text-lg font-semibold text-[#F5F5F5] shadow-sm transition-all hover:bg-[#FFFFFF]/10">
            <span className="inline-flex items-center justify-center gap-2"><MessageCircle size={20} /> Prenota una lezione</span>
          </a>
        </div>
      </div>
