  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 transition-all duration-500 ${mobileMenuOpen ? "z-[9500]" : "z-50"} ${
        isScrolled
          ? "border-b border-[#D8B978]/20 bg-[#2E0A1E] shadow-[0_18px_60px_rgba(61,13,38,0.92)] backdrop-blur-md"
          : "border-b border-[#D8B978]/10 bg-[#3D0D26] backdrop-blur-sm"
      }`}
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-28 items-center justify-between gap-5 py-5 sm:h-32 lg:h-32">
          <a href="#" className="relative z-[60] flex max-w-[260px] flex-col items-start leading-none sm:max-w-[320px]">
            ...
            <span
              className="relative font-serif text-[2.35rem] leading-[0.82] font-semibold tracking-wide text-[#F7EFE5] italic sm:text-[3.05rem]"
          ...
          <div className="hidden items-center gap-8 lg:flex">
                ...
                <a
                  ...
                  className={`relative py-2 text-[12px] font-semibold tracking-[0.16em] uppercase transition-all duration-300 after:absolute after:left-0 after:-bottom-0.5 after:h-px after:bg-white after:transition-all after:duration-300 ${
                    activeSection === link.href.slice(1)
                      ? "text-[#F7EFE5] after:w-full after:opacity-80"
                      : "text-[#F7EFE5]/62 after:w-0 after:opacity-0 hover:text-[#F7EFE5] hover:after:w-full hover:after:opacity-55"
                  }`}
                >
                  {link.label}
                </a>
            ...
            <a
              href="#contatti"
              className="rounded-full border-2 border-white px-6 py-2 font-semibold text-white transition-all hover:bg-white hover:text-[#3D0D26]"
            >
              Iscriviti Ora
            </a>
          </div>
          ...
          <button
            className="z-[9520] rounded-full border border-[#D8B978]/35 bg-[#3D0D26] p-2 text-[#F7EFE5] shadow-lg shadow-black/40 backdrop-blur-md transition-all duration-300 hover:border-[#D8B978]/70 lg:hidden"
      ...
      <div
        className={`fixed inset-0 z-[9500] flex min-h-[100dvh] w-screen items-center justify-center bg-[rgba(61,13,38,0.92)] px-4 py-20 backdrop-blur-xl transition-all duration-300 lg:hidden ${
      ...
      >
        <div
          className={`relative w-[calc(100%-32px)] max-w-[430px] overflow-hidden rounded-[1.35rem] border border-white/20 bg-[#3D0D26] p-6 shadow-[0_24px_70px_rgba(61,13,38,0.92)] transition-all duration-300 sm:p-7 ${
        ...
        >
          ...
          <div className="relative mb-6 border-b border-white/15 pb-6 text-center">
            <p className="text-[8px] font-semibold tracking-[0.38em] text-white/70 uppercase">SCUOLA DI AVVIAMENTO PROFESSIONALE ALLA DANZA</p>
            <p
              className="mt-2 font-serif text-[2.75rem] leading-[0.82] font-semibold tracking-wide text-[#F7EFE5] italic"
              style={{ textShadow: "0 0 28px rgba(216,185,120,0.22)" }}
            >
              Il Sogno
            </p>
            <p className="mt-3 font-serif text-[1.05rem] text-white/80 italic">di Rita Castanò</p>
          </div>
          <div className="relative flex flex-col gap-2.5">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="group flex min-h-14 items-center justify-between rounded-xl border border-white/15 bg-[#1C1416]/88 px-4 py-3.5 text-sm font-semibold tracking-[0.14em] text-[#F7EFE5]/82 uppercase shadow-sm shadow-black/20 transition-all duration-300 hover:border-white/40 hover:bg-[#1C1416] hover:text-[#F7EFE5] active:scale-[0.99]"
                onClick={() => setMobileMenuOpen(false)}
              >
